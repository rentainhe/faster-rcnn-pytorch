...
class Transform(object):
    ...
    def __call__(self, in_data):
        ...
        _, H, W = img.shape
        img = preprocess(img, self.min_size, self.max_size)  # Resize + Normalize
        _, o_H, o_W = img.shape
        scale = o_H / H
        bbox = util.resize_bbox(bbox, (H, W), (o_H, o_W))  # 图片进行了 Resize 操作后对应的 Bounding box也要进行 Resize

        # horizontally flip
        img, params = util.random_flip(
            img, x_random=True, return_param=True
        )
        # params返回 True or False表示是否发生翻转
        bbox = util.flip_bbox(
